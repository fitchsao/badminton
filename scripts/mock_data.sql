...
DELETE FROM matches WHERE court_id IN (
  ...
  WHERE s.id IN (
    SELECT DISTINCT session_id FROM signups WHERE lark_open_id LIKE 'mock_user_%'
  )
);
DELETE FROM court_assignments WHERE signup_id IN (
  SELECT id FROM signups WHERE lark_open_id LIKE 'mock_user_%'
);
DELETE FROM signups WHERE lark_open_id LIKE 'mock_user_%';
DELETE FROM courts WHERE session_id IN (
  SELECT id FROM sessions WHERE event_start_at > NOW() + INTERVAL '5 hours'
                              AND event_start_at < NOW() + INTERVAL '7 hours'
);
DELETE FROM courts WHERE session_id IN (
  SELECT id FROM sessions WHERE event_start_at < NOW() - INTERVAL '29 days'
                              AND event_start_at > NOW() - INTERVAL '31 days'
);
DELETE FROM sessions WHERE event_start_at > NOW() + INTERVAL '5 hours'
                       AND event_start_at < NOW() + INTERVAL '7 hours';
DELETE FROM sessions WHERE event_start_at < NOW() - INTERVAL '29 days'
                       AND event_start_at > NOW() - INTERVAL '31 days';
...
\echo '  - 报名截止: 4 小时后'
\echo '  - 活动开始: 6 小时后'
\echo '  - 已有 15 人报名(11 正式 + 4 候补)'
\echo '  - Fitch 可以再报名一个,会进正式名单'
\echo ''
\echo '历史段位预期:'
\echo '  - Fitch Yu      : 铂金 / 钻石(7 胜 1 负,场均高净胜)'
\echo '  - 陈大力 mock_01 : 黄金 / 铂金'
\echo '  - 李婉婷 mock_02 : 黄金'
\echo '  - 王志强 mock_03 : 白银 / 黄金'
\echo '  - 张芳芳 mock_04..黄建国 mock_07: 4 胜 4 负附近,黄金区间'
\echo '  - 其他 mock_08..15: 待定(无历史)'
\echo '=========================================='
